...
void menu::printMenu()
{
	...
	switch(input)
	{
		...
		case 4:
		{
			...
			if(tmp.getFacultyName() == ("Juniper Ya'Noodle")){
		...
		case 5:
		{
			...
			if(tmp2.getFacultyName() == ("Juniper Ya'Noodle")){
				cout << "Faculty not found." << endl;
			}
			else{
				cout << tmp2 << endl;
			...
			try{
				...
				for(vector<int>::iterator it = tmpAdvisees.begin(); it != tmpAdvisees.end(); ++it) {
					if(*it >=  0){
						...
						if(tmp2.getName() == ("Booboo Man-pro")){
							cout << "Student not found." << endl;
						}
						else {
							cout << tmp2 << endl;
		...
		case 11:
		{
			...
			{
				cout << "Enter valid ID number. " << endl;
				cout << "Enter the new student's ID number: ";
				cin >> id;
				tmp.setStudentID(id);
				tmp = studentBST->searchData(tmp);
				studentExists = studentBST->search(tmp);
			}
			...
			tmp2.setFacultyID(facID);
			tmp2 = facultyBST->searchData(tmp2);
			bool facultyExists = facultyBST->search(tmp2);
			while(!facultyExists)
			{
				cout << "Enter valid ID number. " << endl;
				cout << "Enter the new student advisor's ID number: ";
				...
				tmp2.setFacultyID(facID);
				tmp2 = facultyBST->searchData(tmp2);
				facultyExists = facultyBST->search(tmp2);
			}
			cout << "FACID: " << facID << endl;
			tmp.setStudentAdvisor(facID);
			studentBST->insert(tmp);
			tmp2.addAdvisee(id);
			facultyBST->insert(tmp2);
		...
		{
			commitChange(commitBoth);
			cout << "Enter student's ID number to be removed: ";
			int id;
			cin >> id;
			student tmp;
			tmp.setStudentID(id);
			...
			cout << "Enter the student advisor's ID number: ";
			int facID;
			cin >> facID;
			faculty tmp2;
			tmp2.setFacultyID(facID);
			...
			tmp2.removeAdvisee(id);
			facultyBST->insert(tmp2);
			tmp.setStudentAdvisor(54321);
			studentBST->insert(tmp);
