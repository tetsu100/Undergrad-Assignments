...
public:
	...
	T printTree();
	string serialize();
	string serializeHelper(treeNode<T>* r, string serialBuilder);
...
template <typename T>
string binaryST<T>::serialize()
{
	string serial = "";
	try{
		serial = serializeHelper(root, serial);
	}
	catch(...){
		cout << "ERROR serializing student database." << endl;
	}
}

template <typename T>
string binaryST<T>::serializeHelper(treeNode<T>* r, string serialBuilder)
{
	string tmp = serialBuilder;
